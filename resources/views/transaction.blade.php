...
<head>
  ...
  <title>Transaksi Admin - BukuKu</title>
    ...
    <main class="flex-1 overflow-y-auto">
      ...
      <div class="px-6 md:px-8 py-8 space-y-6">
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-6 md:p-7">
          <h1 class="text-xl md:text-2xl font-semibold text-slate-900">Kelola Transaksi</h1>
          <p class="mt-1 text-sm text-slate-600">Pinjam buku dan kelola pengembalian dengan cepat.</p>
        </section>
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <form action="{{ url('/transactions/pinjam') }}" method="POST" class="mt-5 space-y-4">
            ...
            {{-- ✅ TAMBAH: pilih anggota --}}
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                <label class="block text-xs font-semibold text-slate-600 mb-2">Pilih Anggota *</label>
                <select name="member_id"
                  class="w-full px-3 py-3 rounded-xl border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-slate-300"
                  required>
                  <option value="">-- Pilih Anggota --</option>
                  @foreach($members as $m)
                  <option value="{{ $m->id }}">
                    {{ $m->nama }} ({{ $m->kode_anggota }})
                  </option>
                  @endforeach
                </select>
              </div>
              ...
              <div>
                ...
                <select name="book_id"
                  ...
                  @foreach($books as $b)
                  <option value="{{ $b->id }}">
                    {{ $b->judul ?? $b->title ?? 'Buku' }} (Stok: {{ $b->stok ?? $b->stock ?? 0 }})
                  </option>
                  @endforeach
                </select>
              </div>
